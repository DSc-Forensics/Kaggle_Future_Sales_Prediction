...
metrics={'item_price':['mean','median'],'Revenue':['sum','median']}
...
for col in fit_cols:
    ...
    all_data['_'.join(col.split('_')[:-1])+'_roll6_mean']=(all_data['_'.join(col.split('_')[:-1])+'_1']+\
    all_data['_'.join(col.split('_')[:-1])+'_2']+all_data['_'.join(col.split('_')[:-1])+'_3']+all_data['_'.join(col.split('_')[:-1])+'_4']+\
    all_data['_'.join(col.split('_')[:-1])+'_5']+all_data['_'.join(col.split('_')[:-1])+'_6'])/6.0

del all_data['binned_item_price_itemmean']    
all_data.to_parquet('Inputs/Base_train_v2.gzip',compression='gzip')
